...
let currentFavoriteStatus = false; // 현재 노래의 좋아요 상태

musicAudio.volume=0.5;

//음악 및 음악 정보 불러오기(music_list.js 활용)
const loadMusic=(num)=>{
	favoriteBtn.classList.toggle('active', musicList[num].favorite);
    currentFavoriteStatus = musicList[num].favorite;
...
favoriteBtn.addEventListener('click', (e) => {
    const isFavorite = e.target.classList.toggle('active');

    // 현재 재생 중인 노래의 인덱스
    const currentSongIndex = list_index;

    // 노래의 favorite 속성 업데이트
    musicList[currentSongIndex].favorite = isFavorite;
	currentFavoriteStatus = isFavorite;

    // 여기에 추가적인 동작을 원한다면 할 수 있습니다.
    if (isFavorite) {
        console.log(`Added to favorites: ${musicList[currentSongIndex].name}`);
    } else {
        console.log(`Removed from favorites: ${musicList[currentSongIndex].name}`);
    }
});
...
window.addEventListener('load', () => {
	...
	const initialSongIndex = parseInt(urlParams.get('initialSong')) || 0;
  
	list_index = initialSongIndex;
	loadMusic(list_index);
